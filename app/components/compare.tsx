...
import type { Analysis, Indicator } from '@/src/types';
...
function rowKeys(analyses: Analysis[]): { key: string; label: string }[] {
  ...
  for (const analysis of analyses) {
    for (const indicator of analysis.diagnosis.indicators) {
      if (indicator.group !== 'core') continue;
...
export function Compare({ analyses }: { analyses: Analysis[] }) {
  ...
  const rows = rowKeys(analyses).filter(row => !onlyDifferences || new Set(analyses.map(a => { const i = a.diagnosis.indicators.find(i => i.key === row.key); return JSON.stringify([i?.value, i?.signal === 'na']); })).size > 1);
